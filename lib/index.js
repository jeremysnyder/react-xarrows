...
const Xarrow = (props) => {
    let { id, startAnchor, endAnchor, label, color, lineColor, headColor, strokeWidth, showHead, headSize, showTail, tailColor, tailSize, path, curveness, dashness, animateDrawing, passProps, SVGcanvasProps, arrowBodyProps, arrowHeadProps, arrowTailProps, divContainerProps, SVGcanvasStyle, divContainerStyle, _extendSVGcanvas, _debug, _cpx1Offset, _cpy1Offset, _cpx2Offset, _cpy2Offset } = props, extraProps = __rest(props, ["id", "startAnchor", "endAnchor", "label", "color", "lineColor", "headColor", "strokeWidth", "showHead", "headSize", "showTail", "tailColor", "tailSize", "path", "curveness", "dashness", "animateDrawing", "passProps", "SVGcanvasProps", "arrowBodyProps", "arrowHeadProps", "arrowTailProps", "divContainerProps", "SVGcanvasStyle", "divContainerStyle", "_extendSVGcanvas", "_debug", "_cpx1Offset", "_cpy1Offset", "_cpx2Offset", "_cpy2Offset"]);
    const selfRef = react_1.useRef(null);
    const lineRef = react_1.useRef(null);
    ...
    const [lineLength, setLineLength] = react_1.useState(0);
    ...
    let dashArray, animationDuration, animationRepeatCount, animationStartValue, animationEndValue = null;
    if (animateDrawing) {
    ...
    return (react_1.default.createElement("div", Object.assign({}, divContainerProps, { style: Object.assign({ position: "absolute" }, divContainerStyle) }, extraProps),
        react_1.default.createElement("svg", Object.assign({ id: id, ref: selfRef, width: st.cw, height: st.ch, style: Object.assign({ position: "absolute", left: st.cx0, top: st.cy0, pointerEvents: "none", border: _debug ? "1px dashed yellow" : null }, SVGcanvasStyle), overflow: "auto" }, SVGcanvasProps),
            ...
            showTail ? (react_1.default.createElement("path", Object.assign({ id: `${id}tail`, d: `M 0 0 L ${fTailSize} ${fTailSize / 2} L 0 ${fTailSize} L ${fTailSize / 4} ${fTailSize / 2} z`, fill: tailColor, pointerEvents: "all", transform: `translate(${xOffsetTail},${yOffsetTail}) rotate(${st.tailOrient})` }, passProps, arrowTailProps))) : null,
            react_1.default.createElement("path", Object.assign({ id: `${id}line`, ref: lineRef, d: arrowPath, stroke: lineColor, strokeDasharray: dashArray, strokeWidth: strokeWidth, fill: "transparent", pointerEvents: "visibleStroke" }, passProps, arrowBodyProps), animationSpeed || animateDrawing ? (react_1.default.createElement("animate", { id: `${id}animate`, attributeName: "stroke-dashoffset", values: `${animationStartValue};${animationEndValue}`, dur: animationDuration, repeatCount: animationRepeatCount })) : null),
            showHead ? (react_1.default.createElement("path", Object.assign({ id: `${id}head`, d: `M 0 0 L ${fHeadSize} ${fHeadSize / 2} L 0 ${fHeadSize} L ${fHeadSize / 4} ${fHeadSize / 2} z`, fill: headColor, pointerEvents: "all", transform: `translate(${xOffsetHead},${yOffsetHead}) rotate(${st.headOrient})`, opacity: animateDrawing ? "0" : "1" }, passProps, arrowHeadProps), animateDrawing ? (react_1.default.createElement("animate", { id: `${id}showhead`, dur: "0.05s", attributeName: "opacity", from: "0", to: "1", begin: `${id}animate.end`, repeatCount: "0", fill: "freeze" })) : null)) : null),
...
Xarrow.propTypes = {
    ...
    animateDrawing: prop_types_1.default.oneOfType([prop_types_1.default.bool, prop_types_1.default.string]),
